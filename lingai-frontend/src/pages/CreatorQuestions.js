...
export default function CreatorQuestions() {
  ...
  const handleSave=async()=>{
    if(!form.content.trim()||!form.correct_answer.trim()){toast('Vui lòng điền đầy đủ','error');return;}
    setSaving(true);
    try{
      ...
      toast(editId?'Đã cập nhật!':'Đã tạo — chờ admin duyệt! 🐧');setShowForm(false);load();
    }catch(e){toast(e.message,'error');}finally{setSaving(false);}
  };

  const handleDelete=async id=>{
    if(!window.confirm('Xóa câu hỏi này?'))return;
    try{await questionApi.delete(id);toast('Đã xóa!');load();}catch(e){toast(e.message,'error');}
  };
    ...
    <div className="fade-up">
      <div className="page-header" style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
        <div>
          ...
          <p className="page-sub">Tạo và chỉnh sửa câu hỏi · admin duyệt trước khi hiển thị</p>
        </div>
      ...
      <div className="filter-row">
        ...
        <span style={{fontSize:13,color:'var(--text3)',fontWeight:700}}>{questions.length} câu hỏi</span>
      </div>

      {loading?<div className="loading-page" style={{height:200}}><div className="spinner spinner-lg"/></div>:(
        <div className="data-table-wrap">
          <table className="data-table">
            <thead><tr><th>Nội dung</th><th>Kỹ năng</th><th>Level</th><th>Loại</th><th>Trạng thái</th><th>Hành động</th></tr></thead>
            <tbody>
              {questions.length===0?<tr><td colSpan={6} style={{textAlign:'center',color:'var(--text3)',padding:'32px 0',fontWeight:700}}>
                <img src={IMG_GRAMMAR} style={{width:60,display:'block',margin:'0 auto 10px',opacity:0.4}} alt=""/>
                Chưa có câu hỏi nào
          ...
          <div className="modal-box" onClick={e=>e.stopPropagation()}>
            <div className="modal-header">
              <div style={{display:'flex',alignItems:'center',gap:10}}>
                <img src={IMG_GRAMMAR} style={{width:36,animation:'float 3s ease-in-out infinite'}} alt=""/>
            ...
            <div className="modal-body">
              ...
              <div className="form-group" style={{marginBottom:14}}>
                ...
                <textarea className="form-textarea" rows={3} placeholder="Nhập câu hỏi..."
                ...
                <div style={{marginBottom:14}}>
                  ...
                  {form.options.map((opt,i)=>(
                    <div key={i} style={{display:'flex',gap:8,marginBottom:8,alignItems:'center'}}>
                      ...
                      <input className="form-input" placeholder={`Lựa chọn ${String.fromCharCode(65+i)}`} value={opt} onChange={e=>setOpt(i,e.target.value)}/>
                    </div>
                  ))}
                </div>
              )}
              <div className="form-group" style={{marginBottom:14}}>
                ...
                <input className="form-input" placeholder="Nhập đáp án..." value={form.correct_answer} onChange={e=>setForm(f=>({...f,correct_answer:e.target.value}))}/>
              </div>
              <div className="form-group" style={{marginBottom:14}}>
                <label className="form-label">Giải thích (tùy chọn)</label>
                <textarea className="form-textarea" rows={2} placeholder="Giải thích đáp án..." value={form.explanation} onChange={e=>setForm(f=>({...f,explanation:e.target.value}))}/>
              </div>
            </div>
