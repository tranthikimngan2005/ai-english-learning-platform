...
from app.models.user import ContentStatusEnum, Question, QuestionTypeEnum, ReviewCard, SkillEnum, User
...
def list_library(db: Session, current_user: User, category: str | None = None, difficulty: str | None = None, random: bool = False, shuffle: bool = False, limit: int | None = None) -> list[FlashcardOut]:
    ...
    return [_to_flashcard(review_card, question) for review_card, question in query.all()]


def match_game(db: Session, current_user: User, category: str | None = None, difficulty: str | None = None, random: bool = True, shuffle: bool = False, limit: int = 8) -> list[FlashcardOut]:
    query = _base_query(db, current_user)
    if category:
        query = query.filter(Question.tags.contains(f"category:{_slugify(category)}"))
    if difficulty:
    ...
    return [_to_flashcard(review_card, question) for review_card, question in query.limit(limit).all()]


def list_manage_flashcards(db: Session, current_user: User, category: str | None = None) -> list[FlashcardManageOut]:
    ...
    return [_to_manage_out(question) for question in query.order_by(Question.created_at.desc()).all()]


def create_manage_flashcard(db: Session, current_user: User, payload: FlashcardManageIn) -> FlashcardManageOut:
...
def update_manage_flashcard(db: Session, current_user: User, flashcard_id: int, payload: FlashcardManageIn) -> FlashcardManageOut:
    question = (
        db.query(Question)
...
def delete_manage_flashcard(db: Session, current_user: User, flashcard_id: int) -> None:
    question = (
        db.query(Question)
        .filter(Question.id == flashcard_id, Question.tags.isnot(None), Question.tags.contains("flashcard_vocab"))
        .first()
