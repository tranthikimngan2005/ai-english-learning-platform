...
from app.models.user import ReviewCard, SkillProfile, User
from app.schemas.schemas import DashboardResponse, SkillProfileOut, StreakOut
...
def get_progress(db: Session, current_user: User) -> list[SkillProfileOut]:
    profiles = db.query(SkillProfile).filter(SkillProfile.user_id == current_user.id).all()
    return [
        SkillProfileOut(
            skill=p.skill,
